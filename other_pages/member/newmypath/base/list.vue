<template>
	<scroll-view @scrolltolower="loadData" scroll-y="true">
		<view v-if="goodsList.length > 0">
			<view class="list" @tap="handleDetail(item.goods_id,item.group_id,item.promotion_id)" v-for="(item, index) in goodsList" :key="index">
				<view class="list-imgs">
					<image style="width: 100%; height: 100%;" lazy-load="true" @error="handleErr($event, index)" :src="item.goods_info.picture_img" mode="scaleToFill"></image>
				</view>
				<view class="list-content">
					...
					<view class="price">
						...
						{{ $c.changePrice(item.car_info.ys_price) }}万
					</view>
				</view>
				<image v-if="item.goods_info.state != 1" class="img" src="../../../../static/images/yisx.jpg" mode="scaleToFill"></image>
			</view>
		</view>
...
export default {
	...
	created() {
		this.newMyPath(this.page_index);
	},

	methods: {
		// 页面跳转
		handleDetail(goodsId,group,promotion_id = 0){
			let groupId = parseInt(String(group).substr(0,1));
			this.$c.detailsJump(goodsId,groupId,promotion_id)	
		},

		newMyPath(page_index) {
			...
			MemberApi.newMyPath({
				...
				for (let index in data) {
					data[index].goods_info.picture_img = config.domain + data[index].goods_info.picture_img;
				}
				if(page_index > 1){
